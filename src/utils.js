...
export const fetchSupabaseDB=async()=>{try{const r=await fetch(`${SUPA_URL}cms_store?id=eq.1&select=data&t=${Date.now()}`,{headers:{...getSupaHeaders(),'Cache-Control':'no-store','Pragma':'no-cache'},cache:'no-store'});if(r.ok){const res=await r.json();if(!res||res.length===0){console.warn("Supabase: bảng cms_store trống hoặc không có row id=1");return null;}return res[0]?.data||null;}const errBody=await r.text().catch(()=>'');console.error(`Supabase lỗi HTTP ${r.status}:`,errBody);}catch(e){console.error("Lỗi fetch DB:",e.message);}return null;};
