...
export const encodeBase64UTF8Async = (s) =>
  new Promise((r) => {
    const rd = new FileReader();
    rd.onload = () => r(rd.result.split(",")[1]);
    rd.readAsDataURL(new Blob([s]));
  });
...
export const getHeaders = (t) =>
  t
    ? { Authorization: `Bearer ${t}`, Accept: "application/vnd.github.v3+json" }
...
export const fetchText = async (u, t) => {
  try {
    const r = await fetch(u, {
      headers: { ...getHeaders(t), Accept: "application/vnd.github.v3.raw" },
    });
...
export const getFileShaSafe = async (p, f, t) => {
  try {
    let d = await fetch(
      `https://api.github.com/repos/${p}/contents/${safeEnc(f)}?t=${Date.now()}`,
      { headers: getHeaders(t) },
    ).then((r) => (r.ok ? r.json() : null));
    if (d && !Array.isArray(d)) return d.sha;
    let d2 = await fetch(
      `https://api.github.com/repos/${p}/contents/?t=${Date.now()}`,
      { headers: getHeaders(t) },
    ).then((r) => (r.ok ? r.json() : null));
    if (d2 && Array.isArray(d2)) {
      const x = d2.find((x) => x.name === f);
      if (x) return x.sha;
    }
    return null;
  } catch (e) {
