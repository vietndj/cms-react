  ...
  const renderGridView = (files) => (
      <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-5 w-full">
          {files.map(f => {
              ...
              const tagsList = getFileTags(f.repoName, f.fileName);
              const firstLetter = f.name ? f.name.charAt(0).toUpperCase() : '?';
              const tagColor = tagsList.length > 0 ? getStringColor(tagsList[0]) : 'var(--border)';

              return (
                  <div key={f.sha} className="cms-card flex flex-col relative transition border cms-border hover:border-[var(--accent)] cursor-pointer group shadow-sm bg-[var(--bg-card)] overflow-hidden p-5" onClick={() => window.open(f.url, '_blank')} style={{backgroundColor: col || 'var(--bg-card)', color: textColor, border: `1px solid ${borderColor}`, borderTop: col ? '' : `3px solid ${tagColor}`}}>
                      <div className="flex items-start gap-4 mb-4">
                          {!col && <div className="w-10 h-10 shrink-0 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-sm" style={{ backgroundColor: tagColor }}>{firstLetter}</div>}
                          <div className="flex-1 min-w-0">
                              <h4 className="font-bold leading-[1.4] text-[16px] line-clamp-3" style={{color: textColor}}>{f.name}</h4>
                          </div>
                      </div>

                      {renderTagsAndLinks(tagsList, getFileLinks(f.repoName, f.fileName), btnBg, textColor)}
