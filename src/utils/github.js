...
export const getFileShaSafe = async (repoPath, file, token) => { 
    try {
        let d = await fetchJSON(`https://api.github.com/repos/${repoPath}/contents/${safeEnc(file)}?t=${Date.now()}`, token); 
        if(d && !Array.isArray(d)) return d.sha; 
        let d2 = await fetchJSON(`https://api.github.com/repos/${repoPath}/contents/?t=${Date.now()}`, token); 
